...
LINKERFLAGS = $(GOLDFLAGS)
LINKERFLAGS += -s -w
GORELEASEFLAGS = -ldflags "$(LINKERFLAGS)"
...
compile: swagger
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o bin/${BINARY_NAME}-${VERSION}-linux-amd64 $(GORELEASEFLAGS) main.go
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o bin/${BINARY_NAME}-${VERSION}-darwin-arm64 $(GORELEASEFLAGS) main.go
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o bin/${BINARY_NAME}-${VERSION}-windows-amd64 $(GORELEASEFLAGS) main.go
