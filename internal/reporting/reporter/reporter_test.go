...
import (
	...
	ds_reporter "soarca/internal/reporting/reporter/downstream_reporter"
...
func TestReportWorkflowStart(t *testing.T) {
	...
	wg.Add(1)
	mock_ds_reporter.On("ReportWorkflowStart", runId, playbook, timeNow).Return(nil)
	reporter.ReportWorkflowStart(runId, playbook, mock_time.Now())
...
func TestReportWorkflowEnd(t *testing.T) {
	...
	wg.Add(1)
	mock_ds_reporter.On("ReportWorkflowEnd", runId, playbook, nil, timeNow).Return(nil)
	reporter.ReportWorkflowEnd(runId, playbook, nil, mock_time.Now())
...
func TestReportStepStart(t *testing.T) {
	...
	runId, _ := uuid.Parse("6ba7b810-9dad-11d1-80b4-00c04fd430c8")
	metadata := run.Metadata{RunId: runId, StepId: step1.ID}
...
func TestMultipleDownstreamReporters(t *testing.T) {
	...
	wg.Add(2)
	mock_ds_reporter1.On("ReportWorkflowStart", runId, playbook, timeNow).Return(nil)
	mock_ds_reporter2.On("ReportWorkflowStart", runId, playbook, timeNow).Return(nil)
	...
	wg.Add(2)
	mock_ds_reporter1.On("ReportWorkflowEnd", runId, playbook, nil, timeNow).Return(nil)
	mock_ds_reporter2.On("ReportWorkflowEnd", runId, playbook, nil, timeNow).Return(nil)
