...
import (
	...
	"soarca/internal/reporting/cases/incident"
...
type ICorrelation interface {
	Correlate(run.Metadata, cacao.Variables) (string, error)
}
...
func (correlation *Correlation) GetCaseId(meta run.Metadata, playbook cacao.Playbook) string {
...
func (correlation *Correlation) Correlate(meta run.Metadata, variables cacao.Variables) (string, error) {
	for _, item := range correlation.cases {
		for _, variable := range variables {
			if part, err := item.CheckObservableInCase(variable); err != nil {
				...
				log.Debug(part)
				correlation.addRunToCase(item, meta, variable)
...
// func (correlation *Correlation) addEddxecutionToCase(meta run.Metadata, variables cacao.Variables) error {
// 	return nil
// }
...
func (correlation *Correlation) addRunToCase(item incident.ICase, meta run.Metadata, variable cacao.Variable) {
...
func (correlation *Correlation) createCase(meta run.Metadata, variables cacao.Variables) uuid.UUID {
