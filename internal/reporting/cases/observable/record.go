...
type Runs map[uuid.UUID]MatchReason
...
func (observable *Observable) AddRunToObservable(id uuid.UUID, match MatchReason) {
	if _, ok := observable.Runs[id]; ok {
		return
	}
	observable.Runs[id] = match

}

func (observable *Observable) Match(other Observable, exectuion uuid.UUID) error {
	if len(other.Runs) > 0 {
		log.Warning("the other runs will be discarded when updating the base")
	}
	...
	observable.Runs[exectuion] = reason
