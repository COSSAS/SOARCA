...
const (
	...
	SuccessfullyExecutedText    = "%s run completed successfully"
	FailedText                  = "something went wrong in the run of this %s"
	OngoingText                 = "this %s is currently being executed"
	ServerSideErrorText         = "there was a server-side problem with the run of this %s"
	ClientSideErrorText         = "something in the data provided for this %s raised an issue"
	TimeoutErrorText            = "the run of this %s timed out"
	ExceptionConditionErrorText = "the run of this %s raised a playbook exception"
	AwaitUserInputText          = "waiting for users to provide input for the %s run"
)
...
type StepRunReport struct {
	...
	AutomatedRun bool                      `bson:"automated_run" json:"automated_run"`
	// Make sure we can have a playbookID for playbook actions, and also
	// the run ID for the invoked playbook
}

func RunStatusEnum2String(status runstate_model.Status) string {
	return status.String()
}

// Level must be either "step" or "playbook"
func GetRunStatusText(status string, level string) (string, error) {
	...
	default:
		return "", errors.New("unable to read run information status")
	}
}
