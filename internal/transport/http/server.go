...
import (
	...
	finapi "soarca/internal/transport/http/handlers/fin"
...
func New(ops orchestrator.Operations, opts Options) *Server {
...
func (s *Server) SetupServer() (*gin.Engine, error) {
	...
	api.TriggerRoutes(engine, api.NewTriggerHandler(s.ops.Runs))
	api.StatusRoutes(engine)
	api.PlaybookRoutesWithService(engine, s.ops.Playbooks)
	api.ReporterRoutesWithService(engine, s.ops.Runs)
