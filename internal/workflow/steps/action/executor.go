...
import (
	...
	executors "soarca/internal/workflow/steps"
	"soarca/pkg/cacao"
...
func (executor *Executor) Execute(meta run.Metadata,
...
func (executor *Executor) executeCommandFromArray(meta run.Metadata,
	metadata executors.PlaybookStepMetadata) (cacao.Variables, error) {
	...
	// empty Targets list is a valid shape, not a signal to skip run.
	...
	if len(metadata.Step.OutArgs) > 0 {
		// If OutArgs is set, only update run args that are explicitly referenced
...
func (executor *Executor) evaluateAssignments(extensions cacao.Extensions, results cacao.Variables) cacao.Variables {
	...
	for id, raw := range extensions {
		...
		model, ok := assignment.DecodeAssignment(raw)
		if !ok {
			continue
		}
...
func (executor *Executor) executeCommands(metadata run.Metadata,
